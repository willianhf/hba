...
export class LoginService implements Service<LoginDTO, Output> {
  ...
  public async execute(dto: LoginDTO): Promise<Output> {
    const username = UserName.create({ name: dto.username }).getValue();
    ...
    if (hasActiveSession) {
      throw new ApplicationError('ActiveSessionError', 'Active session error', 'You already have an active session');
    }
